...
(function () {
  ...
  function rendernAnsicht() {
    ...
    document.querySelectorAll(".bw-nav__links a").forEach(function (a) {
      ...
      // Auch Unterrouten markieren: „Gesetze" (#/gesetz/bbig) ist bei jeder
      // #/gesetz/…-Ansicht aktiv.
      if (ziel === aktuell || (aktuell !== "/" && ziel.indexOf(aktuell + "/") === 0)) a.setAttribute("aria-current", "page");
  ...
  // BERUFENET-Suche der Bundesagentur. Die frühere Route „/berufenet/suche
  // ?text=" gibt es seit dem Relaunch der Anwendung nicht mehr — sie liefert
  // im Browser einen 404 (der Server antwortet als Single-Page-App trotzdem
  // mit 200, deshalb fällt das keinem Link-Checker auf). Aktuell ist
  // „/berufenet/ergebnisseite?suchwoerter=". Der Suchbegriff wird auf die
  // Grundform gekürzt: „Gärtner/in" → „Gärtner", „Pferdepfleger/in (§ 66)"
  // → „Pferdepfleger" — sonst findet die Suche den Schrägstrich-Titel nicht.
  function berufenetUrl(titel) {
    var begriff = String(titel || "").split("/")[0].replace(/\s*\(.*$/, "").trim();
    return "https://web.arbeitsagentur.de/berufenet/ergebnisseite?suchwoerter=" + encodeURIComponent(begriff);
  }
    ...
    if (params.b) {
      ...
      if (b) {
        ...
        h += '<div class="export-aktionen">' +
          ...
          '<a class="bw-btn bw-btn--sekundaer" href="' + esc(berufenetUrl(b.titel)) + '" target="_blank" rel="noopener">BERUFENET ↗</a>' +

---
id: "aktenvermerk"
title: "Der Aktenvermerk in der Ausbildungsberatung"
category: "beratung"
category_title: "Ausbildungsberatung & Aufsicht"
summary: "Der Aktenvermerk macht Verwaltungshandeln nachvollziehbar: Wer hat wann mit wem was besprochen, was wurde festgestellt, vereinbart und veranlasst? Dieses Tool erstellt Vermerke strukturiert per Formular."
tags: ["Aktenvermerk", "Vermerk", "dokumentieren", "Gesprächsnotiz", "Akte", "Dokumentation", "Verwaltungshandeln", "Nachweis"]
legal_references:
  - "LVwVfG BW — Grundsätze ordnungsgemäßer Aktenführung"
  - "DSGVO / LDSG BW — Datenschutz bei personenbezogenen Daten"
status: "fachlich geprüft"
reviewed_at: "2026-07-31"
region: "Baden-Württemberg"
source: "Azubi-Wissen v1 (Browsertool des RP Freiburg), Stand Juli 2026"
---
## Das Wichtigste in Kürze

- Ein Vermerk beantwortet: **Anlass — Beteiligte — Sachverhalt — Einordnung — Ergebnis — weiteres Vorgehen.**
- **Zeitnah** verfassen (am selben Tag), sachlich formulieren, **Tatsachen von Bewertungen trennen**.
- **Datensparsamkeit:** nur erfassen, was für den Vorgang erforderlich ist; sensible Daten besonders schützen.
- Vermerke sind **Grundlage für spätere Maßnahmen** (Beanstandung, § 33-Verfahren, Schlichtung) — Lücken schwächen jedes Verfahren.
- Der **Vermerk-Generator** dieses Tools liefert die Struktur und passende Rechtsgrundlagen-Bausteine aus der Wissensdatenbank.

## Aufbau eines guten Vermerks

- **Kopf:** Aktenzeichen, Datum, Verfasser/in, Art (Telefonat, Besuch, Besprechung)
- **Anlass:** Wie kam der Vorgang zustande? (Anruf des Azubis, Schulmeldung, Routinebesuch)
- **Beteiligte:** Personen/Funktionen, Betrieb, Azubi (nur erforderliche Daten)
- **Sachverhalt:** chronologisch, konkret, mit Daten und Zahlen; wörtliche Kernaussagen kenntlich machen
- **Rechtliche Einordnung:** einschlägige Normen und deren Anwendung auf den Fall
- **Ergebnis/Vereinbarung:** Was wurde zugesagt, mit welcher Frist?
- **Weiteres Vorgehen:** Wiedervorlage, Zuständigkeiten, Info an Dritte

## Formulierungsgrundsätze *(ausführlich)*

- Aktiv und präzise: „Der Betrieb sagte zu, ab 01.09. …“ statt „Es wurde besprochen …“
- Bewertungen kennzeichnen: „Aus Sicht der Unterzeichnerin …“
- Keine Polemik, keine Diagnosen, keine Spekulation
- Fremdaussagen im Konjunktiv wiedergeben („Der Azubi berichtet, er müsse …“)
- Anlagen benennen (Berichtsheft-Kopien, Stundenzettel)

So bleibt der Vermerk auch nach Jahren — und vor Gericht — belastbar.

## Praxishinweise

### Für Auszubildende

Auch für dich gilt: Schreib dir wichtige Vorfälle und Gespräche mit Datum auf. Deine Notizen helfen der Beratung, den Fall schnell zu verstehen — und dir, nichts zu vergessen.

### Für Betriebe

Dokumentieren Sie Ausbildungsgespräche und Vereinbarungen kurz schriftlich und geben Sie dem Azubi eine Kopie. Das schafft Verbindlichkeit und schützt bei späteren Streitfragen beide Seiten.

### Für die Ausbildungsberatung

Nutzen Sie den Vermerk-Generator im Export-Bereich: Formular ausfüllen, Themenbausteine mit Rechtsgrundlagen anhängen, als PDF drucken und zur Akte nehmen. Entwürfe bleiben lokal im Browser gespeichert — keine Daten verlassen den Arbeitsplatz.

## Häufige Fragen

### Wann sollte ich in der Beratung einen Aktenvermerk schreiben?

Immer, wenn der Vorgang später nachvollziehbar sein muss: Beschwerden, Betriebsbesuche, Konfliktgespräche, Zusagen mit Frist, Beanstandungen. Faustregel: Was nicht im Vermerk steht, ist nicht geschehen.

## Verwandte Artikel

- Ausbildungsberatung: Überwachen & Beraten (§ 76 BBiG) (`ausbildungsberatung`)
- Konflikte in der Ausbildung lösen (`konflikte`)
- Kündigung des Ausbildungsverhältnisses (`kuendigung`)
